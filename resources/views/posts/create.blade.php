    ...
    <form method="POST" action="{{ route('posts.store') }}">
      ...
      <div class="grid">
        <div style="grid-column:1/-1">
          <label>Tiêu đề</label>
          <input name="title" value="{{ old('title') }}" placeholder="Nhập tiêu đề..." required>
        </div>

        <div style="grid-column:1/-1">
          <label>Tóm tắt (tuỳ chọn)</label>
          <input name="excerpt" value="{{ old('excerpt') }}" placeholder="Tóm tắt ngắn...">
        </div>

        <div style="grid-column:1/-1">
          <label>Nội dung</label>
          <textarea name="content" placeholder="Nhập nội dung..." required>{{ old('content') }}</textarea>
        </div>

        <div>
          <label>Ảnh (link) tuỳ chọn</label>
          <input name="image" value="{{ old('image') }}" placeholder="https://...">
        </div>
        ...
        <div>
          ...
          <select name="status" required>
            ...
            <option value="published" {{ old('status')==='published' ? 'selected' : '' }}>Đăng</option>
          </select>
        </div>
      </div>

      <div class="row">
        <a class="btn2" href="{{ route('posts.index') }}">Huỷ</a>
